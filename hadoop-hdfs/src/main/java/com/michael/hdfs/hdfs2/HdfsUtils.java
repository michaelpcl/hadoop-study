package com.michael.hdfs.hdfs2;
...
public class HdfsUtils {
     ...
     * 读取指定位置的数据
	 */
	public static void readFromHDFSBySeek(String file) throws IOException {
		Configuration conf = new Configuration();
		FileSystem fs = FileSystem.get(URI.create(file), conf);
		Path path = new Path(file);
		FSDataInputStream in = fs.open(path);
	 ...
	 * 用流的形式，上传文件
	 */
	public static void uploadFileByStreaming(String src,String dst) throws IOException {
		Configuration conf = new Configuration();
		FileSystem fs = FileSystem.get(URI.create(dst), conf);

		FileInputStream srcIn = new FileInputStream(src);

		FSDataOutputStream dstOut = fs.create(new Path(dst));

		IOUtils.copyBytes(srcIn,dstOut,4096);

		IOUtils.closeStream(srcIn);
		IOUtils.closeStream(dstOut);
	}
